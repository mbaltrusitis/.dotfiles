...
local function fetchGitStatus(cwd, callback)
  ...
  vim.system(
    ...
    { text = true, cwd = clean_cwd, timeout = 5000 },
    on_exit
  )
end
...
---@param lhs string Left-hand side of the mapping (key combination)
---@param direction string Split direction ('vertical' or 'horizontal')
---@param close_on_open boolean Whether to close mini.files after opening
---@return nil
local function map_split(buf_id, lhs, direction, close_on_open)
  local rhs = function()
    ...
    if cur_target_window ~= nil then
      ...
      MiniFiles.go_in({ close_on_open = close_on_open })
    end
  end
...
autocmd("User", {
  group = augroup("split_mappings"),
  pattern = "MiniFilesBufferCreate",
  callback = function(args)
    local buf_id = args.data.buf_id

    -- Split mappings that close mini.files after opening
    map_split(buf_id, '<C-v>', 'vertical', true)
    map_split(buf_id, '<C-x>', 'horizontal', true)
  end,
})
